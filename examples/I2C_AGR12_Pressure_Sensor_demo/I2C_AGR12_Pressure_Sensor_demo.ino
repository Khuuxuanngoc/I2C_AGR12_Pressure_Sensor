...
/*
    ...
    Y251216: Test I2C AGR12 Pressure Sensor 0~100kPa ASAIR với Vietduino Uno
        ...
        Log:
            - ESP32
                13:25:54.557 -> count: 3
                13:25:54.557 -> Loi CRC! Du lieu khong hop le. CRC nhan: 0, CRC tinh: FF
                13:25:54.557 -> Raw: -256 (0xFF0) | Ap suat: -25.6 kPa
                13:25:54.712 -> count: 3
                13:25:54.712 -> Loi CRC! Du lieu khong hop le. CRC nhan: 0, CRC tinh: FF
                13:25:54.712 -> Raw: -256 (0xFF0) | Ap suat: -25.6 kPa
...
void i2c_50Khz()
{
  ...
  // Arduino Uno (hoạt động ở 16MHz) sử dụng công thức:
  // Tốc độ SCL = Clock_Frequency / (16 + 2 * TWBR * Prescaler)
  Wire.setClock(10000);
  // 1. Khởi tạo I2C bus (Thường mặc định là 100kHz)
  Wire.begin(); 
  // Wire.setClock(50000); // Uno OK
  
  // // 2. Thiết lập Prescaler (TWSR)
  // // Prescaler bits 0 và 1 (TWPS1, TWPS0) đều bằng 0, nghĩa là Prescaler = 1.
  // TWSR &= ~(_BV(TWPS0) | _BV(TWPS1)); 
  
  // // 3. Thiết lập TWBR (Two Wire Bit Rate Register)
  // // Để đạt 50 kHz: 50,000 Hz = 16,000,000 Hz / (16 + 2 * TWBR * 1)
  // // => 16 + 2 * TWBR = 320
  // // => 2 * TWBR = 304
  // // => TWBR = 152
  
  // TWBR = 152; // Thiết lập tốc độ I2C là khoảng 50 kHz
